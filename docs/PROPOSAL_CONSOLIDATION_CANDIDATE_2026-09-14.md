...
# Invisible Ledger — proposal consolidation candidate

**Date:** 2026-09-14  
**Status:** working consolidation for cross-check. **Not advisor-approved and not a replacement for the current DOCX/PDF yet.**  
**Purpose:** consolidate the Kong Sept-01 reviewed rootstock with the Sept-14 empirical proposal while minimizing unnecessary conceptual change. This file deliberately errs toward preserving advisor-reviewed language on the conceptual core and adding new material only where the expanded empirical design requires it.

## Authority and non-negotiable boundaries

Use these authorities in this order:

1. **Kong Sept-01 reviewed draft** — conceptual/rhetorical rootstock.
2. **Current repository data + executed outputs** — empirical and methodological authority.
3. **Sept-14 10-page proposal** — current proposal architecture and latest integrated evidence.
4. **Flashpoint Sept-12** — selective donor only, mainly the accounting/revenue-recognition addition and proposal-level presentation logic.

Do not silently change:

- Indonesia as the proposed main geography;
- the wedge as the transaction–revenue difference rather than missing GDP/income/tax;
- the Ecosystem Ratio as an author-constructed transformation;
- direct/scope-pending/conditional evidence boundaries;
- period/scope matching rules;
- Tokopedia reconciliation into a causal claim;
- BPS aggregate evidence into firm-level causal evidence;
- Bank Indonesia payments into e-commerce sales;
- PMK/DJP legal design into observed compliance or revenue effects;
- global or ASEAN evidence into the Indonesia main sample;
- any candidate sample as advisor-approved.

---

# Candidate consolidated proposal text

## Abstract

This study measures the difference between the transaction value digital platforms process in Indonesia and the revenue those platforms recognize for themselves. I call that difference the **invisible wedge**. The absolute wedge is \(W=V-R\), where \(V\) is transaction value and \(R\) is platform revenue; the **Ecosystem Ratio**, \(E=(V-R)/R\), expresses the same boundary relative to platform revenue. Because no Indonesian platform discloses a strictly country-labelled matched pair across the full study period, observations are retained under named evidence tiers rather than pooled as if they had equal scope. The current inventory spans seventeen candidate platform-year levels across five issuer series from FY2019 to FY2025. Longitudinal evidence shows that transaction value and revenue can move differently, and disclosed revenue components explain important divergences. Official statistics and Indonesia's marketplace reporting rules then provide evidence on wider economic activity and on how platform-held transaction records can enter third-party reporting. External issuer evidence is retained as corroboration rather than as part of the Indonesia sample.
...
Three objectives answer it:

1. **Measure the wedge longitudinally** across retained platform-year candidates while keeping evidence tiers visible.
2. **Explain why the wedge moves** using disclosed monetization, customer-incentive, gross-versus-net revenue, accounting, and business-scope information.
3. **Locate the wider activity and the underlying records beyond platform revenue**, using BPS evidence and Indonesia's third-party reporting / marketplace withholding architecture.
...
Platform revenue and transaction value are taken from primary company disclosures whenever possible. A platform-period is **eligible for consideration** only when:

- transaction and revenue measures cover the same period;
- geography and business scope can be evaluated;
...
- derived inputs trace transparently to source inputs;
- structural breaks, acquisitions and reporting-perimeter changes are flagged;
- repeated publication vintages of the same underlying period are not counted as independent observations.

Eligibility is not equivalent to final main-sample admission. The current analysis therefore keeps three evidence tiers visible:

1. **direct Indonesia-aligned**;
2. **direct scope-pending**;
3. **conditional country reconstruction**.

No expanded sample is represented as advisor-approved.
...
The executed all-tier inventory currently contains **17 matched annual candidate levels and 12 within-series transitions across five series**. A stricter direct-candidate sensitivity gives **9 annual transitions** after the relevant period/scope filters. These are different admissible evidence universes and are reported separately.

### 4.3 Conditional country constructions

Grab's Indonesia transaction value is derived from disclosed Indonesia revenue using the company's Group monetization rate; the assumption is transparent and testable but is not a direct country transaction-value disclosure. Shopee's Indonesia transaction value uses an external market estimate because Sea does not disclose an Indonesia transaction-value figure; every use is labelled accordingly. These constructions are supporting / conditional evidence rather than proof that the same monetization rate holds identically at country level.
...
For the selected FY2023 platform cases, the current construction gives a combined wedge of approximately **US$40.07 billion** and an Ecosystem Ratio of **12.671**. One-at-a-time parameter variation moves the wedge between approximately **US$37.65 billion and US$42.49 billion**. Leave-one-platform-out calculations give approximately **US$35.29 billion, US$24.14 billion, and US$20.70 billion**. These are composition and sensitivity checks, not estimates of one national total.

### 5.2 Two valid longitudinal evidence universes

| Evidence universe | Transitions | Revenue grows faster | Transaction activity grows faster | Opposite-sign transitions | Median absolute divergence |
|---|---:|---:|---:|---:|---:|
| All-tier inventory | 12 | 10 | 2 | 2 | 42.02 pp |
| Direct-candidate sensitivity | 9 | 6 | 3 | 3 | 42.94 pp |

The difference between these summaries comes from admission rules, not from an arithmetic contradiction. The sample hierarchy is therefore shown rather than collapsed.
...
Across **48 matched issuer-years for eight non-Indonesian platform businesses**, the transaction–revenue boundary appears across distinct business models while its magnitude varies substantially. This module is purposively selected on disclosure availability, its definitions differ by issuer, its levels are not pooled, and it does not validate Indonesia country allocations. Its role is external corroboration only.
...
|---|---|---|
| \(V\) | platform transaction value (GTV/GMV/issuer-equivalent transaction metric, scope labelled) | observed or explicitly derived / external by tier |
...
# Cross-check requested from the next reviewer / Claude

Do **not** rewrite immediately. First compare this candidate against:

- `papers/reference/Invisible_Ledger_Sept0126_Kong.docx`;
- `papers/current/Invisible_Ledger_Thesis_Proposal_2026-09-14.docx`;
- `docs/VERSION_SIDE_BY_SIDE_TABLE_2026-09-14.txt`;
- `docs/PROPOSAL_REBUILD_AND_AUDIT_2026-09-14.md`;
- `docs/CURRENT_STATUS.md`;
- `docs/METHODOLOGY.md`;
- `data/README.md`;
- `docs/KNOWN_LIMITATIONS.md`;
- executed outputs and source extracts controlling every number above.

Return a **line-item cross-check** with these labels:

- `SAFE_PRESERVE_ROOTSTOCK`
- `SAFE_NEW_EXTENSION`
- `SUPPORTED_BUT_TOO_PROMINENT`
- `STALE_OR_WRONG`
- `UNSUPPORTED`
- `ADVISOR_DECISION_REQUIRED`

Specifically test:

1. whether restoring Evans & Schmalensee and the ILO sentence creates any citation or scope problem;
2. whether the fuller OECD / DAC7 wording should be retained verbatim before appending PMK;
3. whether the compact claims/non-claims table improves continuity without adding unsupported inference;
4. whether reducing four contributions to three better matches the one-main-question structure;
5. whether naming \(W\) explicitly as the absolute wedge resolves the advisor's construct concern without changing the thesis identity;
6. whether any wording above accidentally upgrades candidate evidence to an approved main sample;
7. whether every empirical number still reproduces from current committed sources;
8. whether any current 10-page wording omitted here should be restored before changing the DOCX/PDF.

**Do not modify the current proposal binary merely because this candidate exists. Cross-check first.**